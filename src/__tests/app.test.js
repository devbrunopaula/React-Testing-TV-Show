...
describe('App Api Async Testing', () => {
  it('App  fetches api for episotes data and renders it', () => {
    render(<Episodes episodes={episodes} />)
  })

  it('App  fetches api for episotes data and renders it', async () => {
    fetchShow.mockResolvedValueOnce(show)
    const {getByTestId, findByText, getByText} = render(<App />)
    const seasonSelection = await waitFor(() => getByText(/Select a season/i))

    fireEvent.click(seasonSelection)

    const app = await waitFor(() => getByTestId('app'))

    expect(app).toBeInTheDOM()
  })
})
